...
import { useMeQuery, useRegisterMutation } from "../../services/userApi";
import GoogleButton from "./GoogleButton";
import { toast } from "react-toastify";
import { consumeAuthReturnTo } from "../../utils/authReturn";

const REFERRAL_STORAGE_KEY = "tradebro_referral_code";
...
const SignUpPage: React.FC<Props> = ({
  ...
  const { data: meRes, isLoading: meLoading, isFetching: meFetching } =
  ...
  const initialReferralCode = queryReferralCode || storedReferralCode;
  ...
  useEffect(() => {
    if (queryReferralCode) {
      setValue("referralCode", queryReferralCode, {
        shouldDirty: false,
        shouldValidate: false,
      });
      window.localStorage.setItem(REFERRAL_STORAGE_KEY, queryReferralCode);
    }
  }, [queryReferralCode, setValue]);
  ...
  useEffect(() => {
    if (mode === "page" && authReady && isAuthenticated) {
      navigate(consumeAuthReturnTo("/profile"), { replace: true });
    }
  }, [mode, authReady, isAuthenticated, navigate]);
  ...
  return (
    ...
    >
      ...
      <div className="relative mx-auto flex min-h-[calc(100vh-140px)] max-w-5xl items-center justify-center px-4 py-10">
        <div className="w-full max-w-md rounded-3xl border border-slate-800/80 bg-slate-900/90 p-6 shadow-[0_20px_70px_rgba(15,23,42,0.9)] backdrop-blur-xl transition-all duration-500 hover:border-emerald-400/70 hover:shadow-[0_0_90px_rgba(16,185,129,0.45)] sm:p-8">
          ...
          <div className="mb-5">
            <GoogleButton
              onSuccess={afterAuth}
              referralCode={referralCodeValue?.trim() || undefined}
            />
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <input
                  type="text"
                  className="w-full rounded-xl border border-slate-700 bg-slate-950/60 px-10 py-2.5 text-sm uppercase text-slate-50 placeholder:text-slate-500 outline-none transition-all duration-200 focus:border-emerald-400 focus:ring-1 focus:ring-emerald-400"
              ...
              <p className="text-[11px] text-slate-500">
                If you opened this page from a referral link, it is already filled.
              </p>
            </div>
            ...
            <button
              ...
              className="mt-1 w-full rounded-xl bg-emerald-400 py-2.5 text-sm font-semibold text-slate-950 shadow-[0_0_30px_rgba(16,185,129,0.7)] transition-all duration-300 hover:-translate-y-0.5 hover:bg-emerald-300 hover:shadow-[0_18px_50px_rgba(16,185,129,0.65)] disabled:cursor-not-allowed disabled:opacity-70"
            >
