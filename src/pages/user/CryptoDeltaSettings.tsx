...
import { Bitcoin, Eye, EyeOff, ShieldCheck } from "lucide-react";
...
export type CryptoDeltaSettingsValue = {
  enabled?: boolean;
  exchange?: "DELTA";
  apiKey?: string;
  apiSecret?: string;
...
export default function CryptoDeltaSettings({
  value,
  onChange,
}: {
  value?: CryptoDeltaSettingsValue;
  onChange: (next: CryptoDeltaSettingsValue) => void;
}) {
  const v = value || {};
  ...
  const completeness = useMemo(() => {
    ...
    const ok = !!(v.apiKey && v.apiSecret);
    return { ok, text: ok ? "Ready" : "Needs API keys" };
  }, [enabled, v]);

  const set = (k: keyof CryptoDeltaSettingsValue, val: any) =>
    onChange({ ...v, [k]: val });
      ...
      <div className={cardBase}>
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-center gap-2">
            <div className="h-9 w-9 rounded-xl bg-yellow-500/15 border border-yellow-400/20 flex items-center justify-center">
              <Bitcoin className="text-yellow-300" size={18} />
            </div>
            <div>
              <h3 className="text-base font-semibold text-slate-100">Crypto Trading</h3>
              <p className="text-xs text-slate-400">
                Execution via Delta Exchange.
              </p>
            </div>
          </div>
        ...
        <div className="mt-5">
          <Toggle
            ...
            hint="Keep enabled if you have an active crypto strategy subscription."
          />
        </div>
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-2">
          ...
          <div className="relative md:col-span-2">
            <label className="text-xs font-medium text-slate-300">API Secret</label>
            <input
              disabled={!enabled}
              className={`${inputBase} ${!enabled ? "opacity-60 cursor-not-allowed" : ""}`}
              placeholder="••••••••"
