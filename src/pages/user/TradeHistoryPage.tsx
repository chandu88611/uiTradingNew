...
const TradeHistoryPage: React.FC = () => {
  ...
  const { data: histData, isLoading, isFetching } = useGetTradesHistoryQuery({
    start: (page - 1) * itemsPerPage,
    count: itemsPerPage,
  });

  const rawTrades = useMemo(() => {
    const a = (histData as any)?.data ?? histData;
  ...
  const trades: Trade[] = useMemo(() => {
    return rawTrades.map((t: any) => ({
      ...
      type:
        String(t.action ?? t.side ?? "BUY").toUpperCase() === "SELL"
          ? "SELL"
          : "BUY",
      quantity: Number(t.volume ?? t.qty ?? 0),
      avgPrice: Number(t.price ?? 0),
      status: (String(t.status ?? "").toLowerCase().includes("complet") ||
      String(t.status).toLowerCase().includes("execut")
        ? "completed"
        : String(t.status).toLowerCase().includes("reject") ||
          String(t.status).toLowerCase().includes("fail")
        ? "rejected"
        : "pending") as Trade["status"],
      broker: String(t.brokerCode ?? t.broker ?? ""),
      timestamp: t.signalTime ?? t.createdAt ?? "",
      pnl: Number(t.pnl ?? 0),
    }));
  }, [rawTrades]);
