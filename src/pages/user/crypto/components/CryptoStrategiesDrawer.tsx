...
import {
  CryptoPlanInstance,
  CryptoPlanSignalSettings,
  CryptoPlanStrategyDef,
  CryptoStrategySelections,
} from "../crypto.types";
...
export default function CryptoStrategiesDrawer({
  open,
  onClose,
  plan,
  strategyDefs,
  ...
  strategyDefs: CryptoPlanStrategyDef[];

  planSignals: CryptoPlanSignalSettings;
  setPlanSignals: (v: CryptoPlanSignalSettings) => void;

  selections: CryptoStrategySelections;
  setSelections: (v: CryptoStrategySelections) => void;
  ...
  const list = useMemo(() => {
    if (!plan) return [];
    return strategyDefs.filter((s) => s.planId === plan.planId && s.market === "CRYPTO");
  }, [plan, strategyDefs]);
  ...
  const setStrategiesEnabled = (v: boolean) => {
    if (!planId) return;
    setPlanSignals({ ...planSignals, [planId]: { ...(planSignals[planId] ?? {}), strategiesEnabled: v } });
  };
  ...
  return (
    <SlideOver
      ...
      subtitle="Enable/disable built-in strategies for this plan. Webhook can be enabled separately."
    >
      {!plan ? (
