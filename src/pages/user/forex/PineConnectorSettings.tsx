...
// /**
//  * ✅ ON/OFF switch:
//  * If status missing => ON.
//  * PENDING_VERIFY => disable.
//  */
...
// const CTRADER_REDIRECT_URI = "https://backend.globalalgotrading.com/ctrader/callback";
...
// /** ✅ Your TradingView webhook endpoint (keep backend/public URL) */

// /** ✅ MT5 EA will poll this (implement on backend) */
// const MT5_BRIDGE_API_BASE = "https://backend.globalalgotrading.com";
...
//  * ✅ FIXED Menu:
//  * - dark popover
//  * - explicit text colors (no inherited white-on-white)
...
// const FOREX_TV_WEBHOOK_URL = `https://backend.globalalgotrading.com/tradingview/alerts?token=${subRes?.data?.webhookToken}`;
...
//   // ✅ MT5 per-account secrets (local for now)
//   const [mt5Secrets, setMt5Secrets] = useState<Record<string, string>>(() =>
//     getLS("fx.mt5Secrets.v1", {})
//   );
...
//   // ensure MT5 secrets for MT5 rows
//   useEffect(() => {
//     let changed = false;
...
//         toast.success(typeUpper === "CTRADER"
...
//                       //   className={clsx(btn, btnOutline, "px-3 py-2")}
//                       // >
//                       //   <Terminal size={14} />
//                       //   MT5 Setup
//                       // </button>
...
//       {/* Create/Edit Account (dark SlideOver) */}
//       <SlideOver
//         open={drawerOpen}
//         onClose={() => {
//           setDrawerOpen(false);
...
//                 After saving, open <b>MT5 Setup</b> on the account card to get TradingView JSON + MQL5 file.
//               </div>
//             </div>
//           )}
...
export default function ForexAccountsPage() {
  ...
  // if LS plan missing (or changed), reset to first available plan
  useEffect(() => {
    if (!plans.length) return;
    const exists = plans.some((p) => p.planId === selectedPlanId);
    if (!selectedPlanId || !exists) setSelectedPlanId(plans[0].planId);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [plans.length]);
  ...
  /**
   * ✅ Trading Accounts now expects planId:
   * GET /trading-accounts?planId=xxxx
   */
  const {
    ...
    selectedPlanId ? ({ planId: selectedPlanId } as any) : (undefined as any),
  ...
  const setAccountEnabled = async (id: number, enabled: boolean) => {
    ...
    try {
      await patchFx({ planId: selectedPlanId, id, patch: { status: toPatchStatus(enabled) } } as any).unwrap();
      toast.success(enabled ? "Account turned ON" : "Account turned OFF");
      refetch();
    } catch (e: any) {
      setEnabledMap((p) => ({ ...p, [key]: !enabled }));
  ...
  const toggleAll = async () => {
    ...
    try {
      const results = await Promise.allSettled(
        actionableRows.map((r) =>
          patchFx({ planId: selectedPlanId, id: Number(r.id), patch: { status: toPatchStatus(next) } } as any).unwrap()
        )
      );
  ...
  const submit = async () => {
    ...
    try {
      ...
      await patchFx({ planId: selectedPlanId, id: editingId, patch } as any).unwrap();
      toast.success("Updated");
      setDrawerOpen(false);
      resetForm();
      refetch();
  ...
  const setMaster = async (id: number) => {
    if (!selectedPlanId) return toast.error("Please select a plan first");
    try {
      await patchFx({ planId: selectedPlanId, id, patch: { isMaster: true } } as any).unwrap();
      toast.success("Master set");
      refetch();
    } catch (e: any) {
      toast.error(e?.data?.message || "Failed");
  ...
  const remove = async (id: number) => {
    if (!selectedPlanId) return toast.error("Please select a plan first");
    try {
      await deleteFx({ planId: selectedPlanId, id } as any).unwrap();
      toast.success("Deleted");
      refetch();
    } catch (e: any) {
      toast.error(e?.data?.message || "Failed");
      ...
      {isLoading ? (
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {rows.map((r) => {
            ...
            const switchDisabled = !!statusBusy[key] || bulkBusy || pendingVerify || (isCtrader && ctraderNeeds);
                  ...
                  <div className="min-w-0">
                    ...
                    {isCtrader && ctraderNeeds ? (
                      <div className="mt-3 rounded-xl border border-yellow-500/20 bg-yellow-500/10 p-3">
                        <div className="text-xs font-semibold text-yellow-200">Action required</div>
                        <div className="text-[11px] text-yellow-100/80 mt-1">
                          Click Verify/Reconnect → login to cTrader → grant access. After redirect back, refresh this
                          page.
                        </div>
                      </div>
                    ) : null}
                ...
                <div className="mt-4 flex items-center justify-between gap-2 flex-wrap">
                  ...
                  <div className="flex items-center gap-2">
                    {isCtrader ? (
                      ...
                      <>
                        {/* keep if you want */}
                        {/* <button type="button" onClick={() => openMt5Setup(r)} className={clsx(btn, btnOutline, "px-3 py-2")}>
                          <Terminal size={14} />
                          MT5 Setup
                        </button> */}
                      </>
        ...
        <div className="space-y-4">
          ...
          {String(fxType).toUpperCase() === "CTRADER" ? (
            <div className="rounded-xl border border-yellow-500/20 bg-yellow-500/10 p-3">
              <div className="text-xs font-semibold text-yellow-200">cTrader token is not entered here</div>
              <div className="text-[11px] text-yellow-100/80 mt-1">
                After saving account, click <b>Verify / Reconnect</b> to grant access on cTrader. Backend callback will
